...
class resnet_proto_IoU(nn.Module):
    def __init__(self, opt):
        ...
        self.dim_dict = {'layer1': 56*56, 'layer2': 28*28, 'layer3': 14*14, 'layer4': 14*14, 'avg_pool': 1*1}
        self.channel_dict = {'layer1': 256, 'layer2': 512, 'layer3': 1024, 'layer4': 2048, 'avg_pool': 2048}
        self.kernel_size = {'layer1': 56, 'layer2': 28, 'layer3': 14, 'layer4': 14, 'avg_pool': 1}
        ...
        # self.cls = nn.Linear(2048, num_fc)
    ...
    def forward(self, x, attribute, return_map=False):
        ...
        if self.avg_pool:
            pre_attri['final'] = F.avg_pool2d(F.conv2d(input=x, weight=self.ALE_vector), kernel_size=14).view(batch_size, -1)
        else:
            att = F.conv2d(input=x, weight=self.ALE_vector)
            pre_attri['final'] = F.max_pool2d(att, kernel_size=14).view(batch_size, -1)
        ...
        # output_final = (pre_attri['final'].mm(attribute))/20.0
        ...
        for name in self.extract:
            ...
            # attention[name] = nn.ReLU()(attention[name])
            # att_shape = attention[name].shape
            # pre_attri[name] = attention[name].view(att_shape[0], att_shape[1], -1).sum(dim=2)
            ...
            # pre_class[name] = (pre_attri[name].mm(attribute))*2

        intermediate = F.avg_pool2d(x, kernel_size=14).view(batch_size, -1)
        # cls = self.softmax(self.cls(intermediate))

        return output_final, pre_attri, attention, pre_class,  intermediate #, cls
